...
describe('contributions count on github', () => {
    it('should get correct contributions', async () => {
        ...
        const totalContributions = await getContributionByYear('KayanShah', 2020, 'token');
    ...
    it('should throw error when api failed', async () => {
        ...
        await expect(getContributionByYear('KayanShah', 2020, 'token')).rejects.toThrow('GitHub api failed');
    });
});
